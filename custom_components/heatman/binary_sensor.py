...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback) -> bool:
    ...
    async_add_entities([HeatmanConnectionSensor(config_entry.runtime_data)] + [HeatmanBinarySensorEntity(config_entry.runtime_data, d).init() for d in config_entry.runtime_data.device.profile.parser.get_entity_descriptions(_PLATFORM)])
...
class HeatmanBinarySensorEntity(HeatmanEntity, BinarySensorEntity):
    def __init__(self, coordinator, sensor):
        HeatmanEntity.__init__(self, coordinator, sensor)
...
class HeatmanConnectionSensor(HeatmanBinarySensorEntity):
