...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback) -> bool:
    ...
    async_add_entities(HeatmanTimeEntity(config_entry.runtime_data, d).init() for d in config_entry.runtime_data.device.profile.parser.get_entity_descriptions(_PLATFORM))
...
class HeatmanTimeEntity(HeatmanWritableEntity, TimeEntity):
    ...
    def native_value(self) -> time | None:
        ...
        try:
            ...
            _LOGGER.debug(f"HeatmanTimeEntity.native_value of {self._attr_name}: {strepr(e)}")
        return None
