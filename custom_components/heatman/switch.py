...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback) -> bool:
    ...
    async_add_entities([HeatmanSwitchEntity(config_entry.runtime_data, d).init() for d in config_entry.runtime_data.device.profile.parser.get_entity_descriptions(_PLATFORM)])
...
class HeatmanSwitchEntity(HeatmanWritableEntity, SwitchEntity):
