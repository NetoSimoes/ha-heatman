...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback) -> bool:
    ...
    async_add_entities(HeatmanDateTimeEntity(config_entry.runtime_data, d).init() for d in config_entry.runtime_data.device.profile.parser.get_entity_descriptions(_PLATFORM))
...
class HeatmanDateTimeEntity(HeatmanWritableEntity, DateTimeEntity):
    ...
    def native_value(self) -> datetime | None:
        ...
        try:
            ...
            _LOGGER.debug(f"HeatmanDateTimeEntity.native_value of {self._attr_name}: {strepr(e)}")
        return None
